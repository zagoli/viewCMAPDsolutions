...
Colors = ['green', 'blue', 'orange', 'yellow', 'pink', 'red', 'purple', 'olive', 'cyan', 'gray']
...
class Animation:
    def __init__(self, my_map, starts, goals, paths):
        ...
        for goal_list in goals:
            self.goals.append([(goal[1], len(self.my_map[0]) - 1 - goal[0]) for goal in goal_list])
        self.paths = []
        if paths:
            for path in paths:
                self.paths.append([])
        ...
        for i, goal_list in enumerate(self.goals):
            for goal in goal_list[::2]:
                self.patches.append(Annulus((goal[0], goal[1]), 0.3, 0.1, facecolor=Colors[i % len(Colors)],
                                    edgecolor='black', alpha=0.5))
        # goal positions
        for i, goal_list in enumerate(self.goals):
            for goal in goal_list[1::2]:
